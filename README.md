...
The results shows that the full probes set is the best model for more than 50% of the genes. interestingly the second best used the most significant probes transformed by a high degree function. To deal with the danger of overfitting we also plot the ratio of test to train error and we found that it is symmetrically distributed. around 1 which indicate that we didn't overfit the data.
