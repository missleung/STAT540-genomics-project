...
The results of this part suggests that firstly, utilizing full set of probes provides the least value of errors in comparison the picking a subset of probes as covariates. Secondly, non-linear transformation of the most significant CpG probe provides less error in comparison the linear model by that CpG probe. For this purpose, we chose two non-linear model to consider the non-linearity of relationship between inputs and outputs. We train a neural network with one hidden layer and at most 5 neurons (due to the computational limitations) and we use logistic function as activation function to involve non-linearity. [Here](https://github.com/STAT540-UBC/Repo_team_Gene_Heroes/blob/master/Step-3-Multiple%20Probe%20Analysis/step3-nonlinear_files/figure-markdown_github/unnamed-chunk-5-1.png), we see the error of neural network model on different settings (number of neurons and regularization function). We see that the model is robust against overfitting and increasing the number of neurons improves the error. A similar diagram for conditioonal inference tree [here](https://github.com/STAT540-UBC/Repo_team_Gene_Heroes/blob/master/Step-3-Multiple%20Probe%20Analysis/step3-nonlinear_files/figure-markdown_github/unnamed-chunk-5-1.png) shows the same trend about the model robustness against overfitting. Consistent with observation in linear models, we see that including all significant CpG probes for a gene (instead of just most significant CpG probe) leads to improvement in gene expression estimation. [here](https://github.com/STAT540-UBC/Repo_team_Gene_Heroes/blob/master/Step-3-Multiple%20Probe%20Analysis/step3-nonlinear_files/figure-markdown_github/unnamed-chunk-6-1.png), we plot the contribution of all two signifcant probes for a random gene in different models showing the contribution of each probe to the overall r-square. We finally compare the mse error of models on test data. While The average error of neural network and conditional inference tree are close (i.e. 0.033 and 0.35) they do not significantly outperform the linear models as we expected initially. In fact the avergae mse for genes across their best fitted model is 0.0358 which is just slightly higher than our neural network. We expect the improvement increases if we increase the number of hidden layers and neurons in the neural network and we are going to adress it in future works.
