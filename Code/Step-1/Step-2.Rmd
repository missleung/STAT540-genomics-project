...
plot(lapply_results_pca_AKT2$num_PC, lapply_results_pca_AKT2$num_less_pvalue)
...
out_genes = lapply(n_genes, function(i){
    ...
    gene_out <- do.call(rbind,out)
    data.frame(num_PC=k, gene = colnames(m[[k]])[i], num_less_pvalue=sum(gene_out$pvalue <= 0.1))
})
...
results_10_genes_average <- results_10_genes %>% group_by(num_PC) %>% summarise(average_sig = mean(num_less_pvalue))
...
out_genes = lapply(n_genes, function(i){
    ...
    gene_out <- do.call(rbind,out)
    data.frame(num_PC=k, gene = colnames(m[[k]])[i], num_less_pvalue=sum(gene_out$pvalue <= 0.05))
})
do.call(rbind, out_genes)
}, mc.cores = 6)
...
g <- ggplot(aes(x=num_PC, y=num_less_pvalue, group=gene, colour=gene), data = results_50_genes)
g + geom_point() + geom_line() + ggtitle("Number of DNA probes less than pvalue = 0.05 on number of PCs adjusted and grouped by gene") + xlab("Number of PCs") + ylab("Number of significant DNA probes for alpha at 0.05")

results_50_genes_average <- results_50_genes %>% group_by(num_PC) %>% summarise(average_sig = mean(num_less_pvalue))
