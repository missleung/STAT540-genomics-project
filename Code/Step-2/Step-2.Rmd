...
saveRDS(results_50_genes_bonerroni_05, "results_50_genes_bonerroni_05_V2.RDS")
...
jpeg("prop_of_genes_sig_on_50_genes.jpg")
p <- ggplot(aes(x=num_PC, y = average_sig), data=results_50_genes_average)
...
With the results on plots above, we decide to adjust for three PCAs.

```{r}
saveRDS(m[[3]], "subjects_genes_PCA_adjusted_V2.RDS")

## using lapply - recommended
results_PCA = parallel::mclapply(1:ncol(m[[3]]), function(i){
    toLoop = which(dat[,i] != 0) 
    out <- lapply(toLoop, function(j){
...
lapply_results_PCA[1:4,]

saveRDS(lapply_results_PCA,"cor_test_results_PCA_lapply_V2.rds")
```
