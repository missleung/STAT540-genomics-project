...
##Create a function called results_by_gene that looks at how many significant probes are there per gene based on pvalue, fdr, and bonferroni 
...
```{r}
lapply_results_pca_AKT2 <- do.call(rbind,results_by_gene("AKT2:ENSG00000105221.10"))
toPlot_lapply_results_pca_AKT2  <- lapply_results_pca_AKT2  %>% melt(id="num_PC")

png("gene_AKT2.png")
ggplot(aes(x=num_PC, y=value, colour=variable), data = toPlot_lapply_results_pca_AKT2) + geom_point() + geom_line( ) + ggtitle("Number of significant eQTMs by number of PCs for AKT2") + xlab("Number of PCs") + ylab("Number of significant DNA probes") + scale_colour_discrete(name = "P-value or FDR", breaks=c("num_less_pvalue", "num_less_fdr"), labels=c("P-value < 0.1", "FDR < 0.1"))
dev.off()
```
...
```{r}
lapply_results_pca_APP <- do.call(rbind,results_by_gene("APP:ENSG00000142192.14"))
toPlot_lapply_results_pca_APP <- lapply_results_pca_APP %>% melt(id="num_PC")

png("gene_APP.png")
ggplot(aes(x=num_PC, y=value, colour=variable), data = toPlot_lapply_results_pca_APP) + geom_point() + geom_line( ) + ggtitle("Number of significant eQTMs by number of PCs for APP") + xlab("Number of PCs") + ylab("Number of significant DNA probes") + scale_colour_discrete(name = "P-value or FDR", breaks=c("num_less_pvalue", "num_less_fdr"), labels=c("P-value < 0.1", "FDR < 0.1"))
dev.off()
```
